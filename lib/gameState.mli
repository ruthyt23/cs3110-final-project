...
val play_card :
  game_state -> Player.player -> Deck.card -> bool -> game_state * int
(** [play_card game_state card player] plays the given [card] for the specified
    ...
    removed from the player's hand in the updated game state - the number of
    additional cards played (e.g. Just Say No, Double the Rent) is also
    returned. *)
